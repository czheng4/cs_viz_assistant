...
function change_color(dict) {
  ...
  */
  target.fillStyles[3] = "pink";
  target.fillStyles[0] = "lightblue";
}
...
function example1() {
 ...
 // Rect contructor (x, y, width, height, ref = "", text = [], label = "", label_position = "top", text_direction = 'v', ctx_prop = {}) {
  ...
  let rect = new Rect(0, 100, 500, 100, 0, [23, 54, 53, 3432, 43], "Rect1", "bottom", "h");
  ...
  let t = new Text("This is test", 10, 10, 100);
  ani.add_object(t);
  ...
  let rev_change_color = function (rect) {
    console.log(rect);
    for(let i = 0; i < 5; i++)
    rect.fillStyles[i] = "#DDDDDD";
    // console.log(rect.fillStyles);
  }
  ...
  ani.add_sequence_ani({
    pause:10,
    rev_action: {params: rect, func: rev_change_color},
  });
