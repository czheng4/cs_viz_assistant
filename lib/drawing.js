...
class quadraticCurve {
  ...
  draw(skip_animation = false) {
      ...
      for (i = 0; i < this.animations.length; i++) {
        ...
        if (p.ani_time >= p.start && p.ani_time <= p.end) {
          ...
          if ( (p.ani_time == p.end && !REVERSE_ANIMATION) || 
            ...
            console.log(p);
            if ("fade_out" in p) {
              this.alpha = 1 - (p.ani_time - p.start + 1) / p.time;
            }
            if ("fade_in" in p) {
              this.alpha = (p.ani_time - p.start + 1) / p.time;
            }
    ...
    if ("tmp_p" in this) {
      this.cp = this.control_point(this.p1, this.tmp_p, this.h_scale, this.w_scale);
      ctx.quadraticCurveTo(this.cp.x, this.cp.y, this.tmp_p.x, this.tmp_p.y);
    } else {
      this.cp = this.control_point(this.p1, this.p2, this.h_scale, this.w_scale);
