      ...
      if (this.edge_line != null) {
        ...
        for (let obj of this.obj_map.values()) {
          if (obj != this.dragging_obj && "mouse_down" in obj && obj.mouse_down(x,y)) {
            ...
            w = prompt("Please enter the weight of edge");
            w = parseFloat(w);

            if (!isNaN(w) && w > 0) {
              this.g.get_edge(this.g.get_node(this.dragging_obj.ref), this.g.get_node(obj.ref), w);
            }
            obj.mouse_up();
            PRESS_E_KEY = false;
            break;
...
class Graph {
  ...
  get_edge(n1, n2, weight = 0) {
    ...
    if (id in this.edge_map) {
      ...
      if (weight != 0) {
        e.weight = weight;
        if (e.ani_line != null) e.ani_line.text = weight;
