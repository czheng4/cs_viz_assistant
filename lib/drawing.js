...
class Animation {
        ...
        for (let obj of this.obj_map.values()) {
          if (obj != this.dragging_obj && "mouse_down" in obj && obj.mouse_down(x,y)) {
            ...
            if (this.press_key == 'W') { // add
              ...
              // if (w == null || w == "") {
              //   // if we release 'w' key while the prompt is open. The keyup event is not tiggered.
              //   PRESS_W_KEY = false;
              // }
  ...
  mouse_move(x, y) {
    ...
    if (this.dragging_obj != null) {
      ...
      if ( (PRESS_W_KEY || PRESS_D_KEY) && !this.during_animation) {

        if (this.g != null && !this.g.enable_edge_create) return;
