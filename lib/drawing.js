  ...
  */
  link_text(wait = false) {
    ...
    for (i = 0; i < keys.length; i++) {
      ...
      // console.log("closed" in SCRIPT);
      if (SCRIPT != null && SCRIPT.closed === false) {
        if (i == 0 && SCRIPT.document.body != undefined) {
          SCRIPT.document.body.innerHTML = "";
        }
        script_table(i, pre_text);
        // if (dict[keys[i]].start == this.start && !REVERSE_ANIMATION)  highlight_script_block(i);
        // if (dict[keys[i]].end == this.start && REVERSE_ANIMATION) highlight_script_block(i - 1);
        // SCRIPT.document.write("<p id = script_{} >{}</p>".format(i, pre_text));
      }

    }
    console.log(this.script_cur_index);
    highlight_script_block(this.script_cur_index, wait);
    // let id = SCRIPT.document.getElementById("script_0");
    // id.style.color = "red";
