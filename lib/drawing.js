...
class Animation {
  ...
  set_animation_property(ani_ps, execute_ani_ps) {
    let time = 0;
    let ani_p;
    let i;

    if (ani_ps.length == 0) return 0;

    for (i = 0; i < ani_ps.length; i++) {
      ani_p = ani_ps[i];
      ...
      if ('pause' in ani_p) { 
        ...
        time = parseInt(ani_p.pause);
        if (time <=0 ) time = 1;
        break;
      }
      if (!('time' in ani_p.prop)) ani_p.prop.time = ANIMATION_TIME;
      if (time < ani_p.prop.time) time = ani_p.prop.time 
      
      if ("target" in ani_p) ani_p.target.set_animation_property(ani_p.prop);
      execute_ani_ps.push(ani_p);
      if ('concurrence' in ani_p && ani_p.concurrence == true) {
  ...
  run_animation(skip = false) {
    let count, time, acc_count;
    let execute_ani = [];
    let self = this;
    let ani_p;
    let i;
    let parallel_ani_done = 0;


   
    time = this.set_animation_property(this.sequence_ani, execute_ani);
    count = 0;
    acc_count = 0;
    ...
    let id = setInterval(function() {
      ...
      if (count == time) {
        ...
        // sequence animation call back
        for (i = 0; i < execute_ani.length; i++) {
          ani_p = execute_ani[i];
          if (ani_p.prop.time == count) {
            if ("action" in ani_p) {
        ...
        if (self.sequence_ani.length > 0 && !("pause" in self.sequence_ani[0])) self.suspend = true;
        time = self.set_animation_property(self.sequence_ani, execute_ani);
        

        // console.log("done");
      }

      if (time == 0 && parallel_ani_done == self.parallel_ani.length) {
        self.draw();
        console.log("done");
