...
class quadraticCurve {
  constructor(p1, p2, h_scale = 1/3, w_scale = 0.5, draw_arrow = true, text = "", ctx_prop = {} ) {
    this.p1 = p1;
    this.p2 = p2;
  ...
  draw_text() {
    ...
    // console.log(this.text, p.x + (this.width - tw) / 2, p.y - 20)
    ctx.strokeText(this.text, p.x + 5, p.y - 10);

  }
  draw_angle(position = 1, angle_length = 20, angle_degree = 30, reverse = false) {
    if (this.p1.is_euqal(this.p2)) return;
    let coord, coord1;
    let line;
    if (position == 0) {
  ...
  draw () {
    ...
    if (this.label != "") {
      if (this.label_position == "top") {
        this.draw_text(this.x, this.y - 2 * PADDING, this.width, this.label);
      } else if (this.label_position == "bottom"){
        this.draw_text(this.x, this.y + this.height + 2 * PADDING, this.width, this.label);
      } else {
