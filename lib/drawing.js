...
var ANIMATION_TIME = 100; // controls the animation speed. The slower the animation, the bigger it is.
const PADDING = 10;
...
/* Dynamicly change animation time 
   If any of arg is "ANIMATION_TIME", it will be replaced with run time value.
   op is one of "+ - * /"
*/
class AniTime {
...
class animationTester {
  ...
  /* This is for dependent animation where each state is stored */
  run_test() {
  ...
  /* This is for independent animation where each state is not stored */
  run_non_continuous_test() {
...
class Animation {
  ...
  constructor(elaboration_text = null) {
    ...
    this.play_animation_test = false; // for testing only. it sinmply show the entire animation. See animationTester
  ...
  */
  ...
  process_parallel_ani() {
    ...
    /* convert time to dynamic AniTime */
    ...
    */
    ...
    for (i = 0; i < this.sequence_ani.length; i++) {
      ...
      if ("pause" in seq) {
        ...
        time = new AniTime("ANIMATION_TIME");
      }
  ...
  run_animation(skip = false, callback = null) {
    ...
    let id = setInterval(function() {
      ...
      */
        ...
        */
        ...
        for (i = 0; i < self.parallel_ani.length - 1; i++) {
          ...
          if (ani_p.prop.start == self.start) {
            ...
            ani_p.prop.time = ani_p.prop.end - ani_p.prop.start;
            if ("target" in ani_p) {
      ...
      if (REVERSE_ANIMATION == true) self.start--;
      else self.start++;
