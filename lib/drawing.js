...
/* dynamicly change animation time */
class AniTime {
  constructor(arg1 = "", op = "", arg2 = "") {
    this.arg1 = arg1;
    this.op = op;
    this.arg2 = arg2;
  }
  ...
  get_time() {
    ...
    if (this.arg1 == "ANIMATION_TIME") i = ANIMATION_TIME;
    else i = parseInt(this.arg1);

    // only arg1 is specified
    if (this.op == "") {
      if (isNaN(i)) {
        console.log(this.arg1 + " is not valid");
        return -1;
      } else return i;
    }

    if (this.arg2 == "ANIMATION_TIME") j = ANIMATION_TIME;
    else j = parseInt(this.arg2);

    if (isNaN(j) || isNaN(i)) {
      console.log(this.arg1 + " or " + this.arg2 + " is not valid");
      return;
    }

    if (this.op == "/") rv = i / j;
    else if (this.op == "*") rv = i * j;
    else if (this.op == "-") rv = i - j;
    else if (this.op == "+") rv = i + j;
  ...
  static int_to_ani_time(time) {
    if (typeof time == "number") {
      return new AniTime(time);
...
class animationTester {
  constructor(algorithm, speed = 50) {
    this.algorithm = algorithm;
    this.ani = this.algorithm.ani;
    this.speed = speed;

    if (this.speed < 90 && this.speed >= 0) ANIMATION_TIME = 100 - this.speed;
  }
  ...
  add_func(name, args = []) {
    ...
    d.name = name;
    d.args = args;
    this.ani.function_calls.push(d);
  }

  run_test() {
    this.ani.play_animation_test = true;
    this.ani.call = this.ani.function_calls[0];
    this.ani.resume_animation();
  }
}
...
class Animation {
  ...
  constructor(elaboration_text = null) {
    ...
    // for testing only. it sinmply show the entire animation 
    this.play_animation_test = false;

    /* function_calls and all_anis are shared throughtout all instance of Animation */
  }
  ...
  refresh_state (ani) {
    ...
    this.connection_map = ani.connection_map;
    this.obj_map = ani.obj_map;
    this.points = ani.points;
  ...
  restore_state() {
    ...
    console.log(this.running_ani_index);
    ...
    if (MAIN_A.g != null) MAIN_G = MAIN_A.g;
    ...
    console.log(bookkeeping.staring_time, this.start);

    if (bookkeeping.staring_time >= this.start) {
      console.log("resote", this.running_ani_index);
      console.log(bookkeeping.state);
      MAIN_A = bookkeeping.state;
      ...
      console.log(MAIN_A.ani);
      // this.all_anis.splice(this.running_ani_index, this.all_anis.length - this.running_ani_index);
      // console.log(this.all_anis);
    } else if (bookkeeping.ending_time <= this.start && this.running_ani_index + 1 < this.all_anis.length) {
      bookkeeping = this.all_anis[this.running_ani_index + 1];
      bookkeeping = bookkeeping[bookkeeping.length - 1];
      MAIN_A = bookkeeping.state;
  ...
  process_parallel_ani() {
    ...
    for (i = 0; i < this.parallel_ani.length; i++) {
      ...
      p.prop.start = AniTime.int_to_ani_time(p.prop.start);
      p.prop.end = AniTime.int_to_ani_time(p.prop.end);

      p.prop.start_d = p.prop.start;
      p.prop.end_d = p.prop.end;
      

      p.prop.start = p.prop.start.get_time();
      p.prop.end = p.prop.end.get_time();
  ...
  process_sequence_ani() {
    ...
    // if (this.sequence_ani.length == 0) return;
    /* bookkeeping is the last element of parallel_ani array
       ...
       The index of this animation in all_anis, ending_time and starint_time and the state of animation. 
       The state is the state of algorithm before we run this animation. 
    */
    ...
    for (i = 0; i < this.sequence_ani.length; i++) {
      ...
      } else time = new AniTime("ANIMATION_TIME");

      time = AniTime.int_to_ani_time(time);
      seq.prop.time = time;
      // console.log(time);
      seq.prop.end = start + time.get_time();
    ...
    bookkeeping.staring_time = this.start;



    this.parallel_ani.push(bookkeeping);
    // this.running_ani_index += 1; 
    // this.all_anis.length;
    
    // if (this.all_anis.length == 0 || this.running_ani_index == this.all_anis.length - 1) {
    ...
    // } else {
    //   this.running_ani_index += 1;
    //   this.all_anis[this.running_ani_index] = this.parallel_ani;
    //   // this.running_ani_index += 1;
    // }
  ...
  resume_animation() {
    ...
    if (!this.reverse && this.call != null && "name" in this.call) {
      func = this.call;
  ...
  run_animation(skip = false, callback = null) {
    ...
    if (self.parallel_ani.length == 0) {
      console.log(self.parallel_ani);

      throw new Error("no parallel_ani");
      return;
    }
    this.suspend = false;
    this.call = null;
    ...
    console.log(self.parallel_ani, self.call);
    ...
    let id = setInterval(function() {
      ...
      */
      if (self.start > max || self.start < min) {
        console.log("race conditon occurs");
        clearInterval(id);
      }
      ...
      if (self.reverse == false) {
        ...
        for (i = 0; i < self.parallel_ani.length - 1; i++) {
          ...
          if (parseInt(ani_p.prop.end) == self.start && !self.has_suspend) {
            if ("action" in ani_p) {
        ...
        for (i = 0; i < self.parallel_ani.length - 1; i++) {
          ...
          if (parseInt(ani_p.prop.start) == self.start) {
            ...
            if ("target" in ani_p) {
              ...
              if ("ani" in ani_p.prop) {
                console.log("set ani ", self);
                ani_p.prop.ani = self;
              }
        ...
        for (i = 0; i < self.parallel_ani.length - 1; i++) {
          ani_p = self.parallel_ani[i];
          if ("prop" in ani_p) ani_p.prop.ani_time = self.start;
          if (parseInt(ani_p.prop.start) == self.start && !self.has_suspend)  {
      ...
      if (!self.reverse && parseInt(max) == self.start) {
        self.draw();
        ...
        if (self.running_ani_index < self.function_calls.length - 1) {
          ...
          stop_between = true;
          self.parallel_ani = [];
          if (self.play_animation_test) self.resume_animation();
      ...
      /* go to previous episode of animation */
      if (self.reverse && min == self.start) {
        self.start--;
        self.draw();
        ...
        // self.call = self.function_calls[self.running_ani_index];
        // console.log(self.running_ani_index);
      ...
      // console.log(min, max, self.start);
        ...
        if (p.ani_time >= p.start && p.ani_time <= p.end) {
          ...
          if ( (p.ani_time == p.end && !REVERSE_ANIMATION) || 
            ...
            if ("p" in p) { 
              ...
              line = p.path;
              if (REVERSE_ANIMATION) {
                t = (p.ani_time - p.start)  / p.time;
              } else {
                t = (p.end - p.ani_time) / p.time;
              }
        ...
        if (p.ani_time >= p.start && p.ani_time <= p.end) {
          ...
          if ( (p.ani_time == p.end && !REVERSE_ANIMATION) || 
            ...
            if ("p" in p) { 
              ...
              if (REVERSE_ANIMATION) t = p.end - p.ani_time;
              else t = p.ani_time - p.start;
              ...
              if (!REVERSE_ANIMATION) {
                this.copy_to_entry((p.ani_time - p.start) / p.time, 
...
class Graph {
  constructor(ani, graph_type = "undirect") {
    ...
    // these spec may differ for differetn animations.
  ...
  get_edge(n1, n2, weight = 0, force = true) {
    ...
    if (id in this.edge_map) {
      ...
      if (this.graph_type == "undirect") {
        if (force) this.edge_map[id2].weight = weight;
      }
