...
class Animation {
  ...
  deep_copy() {
    ...
    for (let [key, value] of this.connection_map) {
      let v = [];
      for (let i = 0; i < value.length; i++) v.push(value[i]);
  ...
  add_parallel_ani(p) {
    ...
    /* store dynamic time. It changes as we change ANIMATION_TIME */
    //  console.log(p.prop.start, p.prop.end);
  ...
  process_sequence_ani() {
    ...
    */
    ...
    this.running_ani_index = this.all_anis.length;
  ...
  resume_animation() {
    ...
    let bookkeeping = this.all_anis[this.running_ani_index];
    // console.log(this.all_anis.length);
    bookkeeping = bookkeeping[bookkeeping.length - 1];

    console.log(bookkeeping.staring_time, bookkeeping.ending_time, this.start);
    if(this.reverse && bookkeeping.staring_time >= this.start) {
      this.running_ani_index -= 1;
    }
    if (!this.reverse && bookkeeping.ending_time <= this.start) {
      this.running_ani_index += 1;
    }

    console.log(this.running_ani_index);
    this.parallel_ani = this.all_anis[this.running_ani_index];
    console.log(this.during_animation, this.start);
  ...
  run_animation(skip = false, callback = null) {
    ...
    let id = setInterval(function() {
      ...
      if (!self.reverse && parseInt(max) == self.start) {
        ...
        console.log(self.running_ani_index, self.all_anis.length - 1);
        ...
        if (self.running_ani_index < self.all_anis.length - 1) {
    ...
    if (!skip_animation) {
      for (i = 0; i < this.animations.length; i++) {
        ...
        if (p.animation_time > 0) {
          ...
          if ( (p.animation_time == p.time || p.animation_time == 0) && p.type == 'pivot' && ("ani" in p)) {
            
            console.log("get here", p);
            // if (p.ani.g != null) p.ani.g.remove_edge_by_name(this.p1.obj.ref, this.p2.obj.ref);
            // else p.ani.disconnect_object(this);
