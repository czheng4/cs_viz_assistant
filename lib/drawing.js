...
const BLUE_SPAN = '<span style = "color: blue; font-weight:bold">';
const RED_SPAN = '<span style = "color: red; ">';
const PADDING = 10;
...
const DEFAULT_TEXT_CTX = {fillStyle: "black"};
const ROUNDING_THRESHOLD = 0.001;
...
var DISABLE_IDS = [];
var ENABLE_STEP_IDS = [];
var DISABLE_STEP_IDS = [];
var START_IDS = []; // get to the beginning of animation
var SCRIPT = null;
...
// make a script table
function script_table(step_num, text) {

  text = text.replaceAll(NEW_LINE, "<br>");
  let str = "<table class = script-table> \
               <tr> \
                 <td class = script-table-td1>{}</td> \
                 <td class = script-table-td2 id = script_{}>{}</td> \
               <tr> \
             </table>".format(step_num, step_num, text);

  SCRIPT.document.write(str);
}

function highlight_script_block(step_num, wait = false) {
  if (step_num >= 0 && SCRIPT != null && SCRIPT.closed === false) {
    let pre_id;
    let id = SCRIPT.document.getElementById("script_" + step_num);

    if (!REVERSE_ANIMATION) pre_id = SCRIPT.document.getElementById("script_" + (step_num - 1));
    else pre_id = SCRIPT.document.getElementById("script_" + (step_num + 1));

    if (pre_id != null) pre_id.style.backgroundColor = "white";
    if (id != null) {
      id.style.backgroundColor = "#e8e8e8";
      //console.log("scroll");
      if (wait == true) setTimeout(function(){ id.scrollIntoView({block: "center"}); }, 500);
      else id.scrollIntoView({block: "center"});
    }
...
class animationTester {
  constructor(speed = 50, wait_begin = 0, wait_between = 0) {

    this.speed = speed;
    this.function_calls = [];
    this.wait_begin = wait_begin;
    this.wait_between = wait_between;
  ...
  run_test() {
    ...
    let self = this;
    let n, wait;
    wait = this.wait_between;
    
    n = 0;

    if (this.function_calls.length == 0) return;
    //console.log(this.function_calls.length)
    id = setInterval(function(){
      let func;
      ani = MAIN_A.ani;
      if (n < self.wait_begin) {
        n++;
        return;
      }
      ...
      if (wait < self.wait_between) {
        wait++;
        return;
      }
      D_ANIMATION_TIME = 100 - self.speed;
      wait = 0;
...
class Animation {
  ...
  */
  ...
  last_seqence_ani() {
    if (this.sequence_ani.length == 0) return null;
    return this.sequence_ani[this.sequence_ani.length - 1];
  }
  is_last_sequence_ani_step() {
    let p = this.last_seqence_ani();
    if (p == null) return true;
    if ("step" in p.prop && p.prop.step == true) return true;
    return false;
  }
  ...
  process_parallel_ani() {
    ...
    for (i = 0; i < this.parallel_ani.length; i++) {
      ...
      p.prop.start = parseInt(p.prop.start);
      p.prop.end = parseInt(p.prop.end);
  ...
  */
  process_sequence_ani() {
    ...
    /* bookkeeping is the last element of parallel_ani array
       ...
       The index of this animation in all_anis, ending_time and starting_time and the state of animation. 
    ...
    */
    ...
    for (i = 0; i < this.sequence_ani.length; i++) {
      ...
      if ("pause" in seq) {
        time = parseInt(seq.pause);
      } else if ("time" in seq.prop) {
        time = parseInt(seq.prop.time);
      } else {
        time = ANIMATION_TIME;
      }
      ...
      if ("time_offset" in seq) {
        seq.time_offset = parseInt(seq.time_offset);
        seq.prop.start += seq.time_offset;
        seq.prop.end += seq.time_offset;
      }
    ...
    /* sort the animations based on the starting time */
    this.parallel_ani.sort(function(a, b) { return a.prop.start - b.prop.start;  });
    // for (i = 0; i < this.parallel_ani.length; i++) //console.log(this.parallel_ani[i], this.parallel_ani[i].prop.start);
  ...
  write_script(text = [], highlight_index = -1) {

    if (SCRIPT != null && SCRIPT.closed === false) {
      for (let i = 0; i < text.length; i++) {
        if (i == 0 && SCRIPT.document.body != undefined) {
          SCRIPT.document.body.innerHTML = "";
        }
        script_table(i, text[i]);
      }
    }
    if (highlight_index >= 0 && highlight_index < text.length) highlight_script_block(highlight_index);
  }
  /* store the previous elaboration text
     It's used to show the previous elaboration text when going back.
  */
  link_text(wait = false, reverse = false) {
    let i;
    let start, ani_p;
    let dict = {};
    let keys, pre_text;
    ...
    for (i = 0; i < this.parallel_ani.length; i++) {
       ...
       if ("text" in ani_p) {
        ani_p.pre_text = pre_text;
        ani_p.script_index = index;
        pre_text = ani_p.text;
        

        if (SCRIPT != null && SCRIPT.closed === false) {
          if (index == 0 && SCRIPT.document.body != undefined) {
            SCRIPT.document.body.innerHTML = "";
          }
          script_table(index, pre_text);
        }
    ...
    if (reverse == true) {
      this.elaboration.text("");
      this.elaboration.append(pre_text);
      highlight_script_block(index - 1);
    } else {
      highlight_script_block(this.script_cur_index, wait);
    }
    return;
    ...
    /* get prev text */
    if (keys.length != 0) pre_text = "";

    //console.log("call link text: ", TMP_COUNT++);
    for (i = 0; i < keys.length; i++) {
      dict[keys[i]].pre_text = pre_text;
      dict[keys[i]].script_index = i;
      ...
      if (SCRIPT != null && SCRIPT.closed === false) {
        ...
        script_table(i, pre_text);
      }

    }
    //console.log(this.script_cur_index);
    highlight_script_block(this.script_cur_index, wait);
  ...
  resume_animation() {
    ...
    /* otherwise the animation is running step-by-step */
      ...
      bookkeeping = this.parallel_ani[this.parallel_ani.length - 1];
      
      //console.log(bookkeeping.ending_time, this.start)
      if (REVERSE_ANIMATION && bookkeeping.ending_time == this.start - 1) {
        this.link_text(false, true);
      }
  ...
  */
  run_animation(callback = null) {
    ...
    if (this.parallel_ani.length <= 1) return;
    ...
    let id = setInterval(function() {
      ...
      if(REVERSE_ANIMATION && self.start == - 1) {
        ...
        for (i = 0; i < START_IDS.length; i++) $(START_IDS[i]).prop("disabled", false);
        clearInterval(id);
        return;
      }
        ...
        for (i = 0; i < self.parallel_ani.length - 1; i++) {
          ...
          if (ani_p.prop.end == self.start && !self.has_suspend) {
            ...
            if ("action" in ani_p) {
              ...
              if (typeof ani_p.action.params == "object" && "ani" in ani_p.action.params) ani_p.action.params.ani = self;
              if (typeof ani_p.action.params == "object" && "g" in ani_p.action.params) ani_p.action.params.g = MAIN_G;
        ...
        for (i = 0; i < self.parallel_ani.length - 1; i++) {
          ...
          if (ani_p.prop.start == self.start) {
            ...
            if ("text" in ani_p) {
              ...
              highlight_script_block(ani_p.script_index);
              self.script_cur_index = ani_p.script_index;
            }
        ...
        for (i = 0; i < self.parallel_ani.length - 1; i++) {
          ...
          if (ani_p.prop.start == self.start && !self.has_suspend)  {
            ...
            if ("rev_action" in ani_p) {
              ...
              if (typeof ani_p.rev_action.params == "object" && "ani" in ani_p.rev_action.params) ani_p.rev_action.params.ani = self;
              if (typeof ani_p.rev_action.params == "object" && "g" in ani_p.rev_action.params) ani_p.rev_action.params.g = MAIN_G;
        ...
        for (i = 0; i < self.parallel_ani.length - 1; i++) {
          ...
          if (ani_p.prop.end == self.start) {
            ...
            if ("pre_text" in ani_p) {
              ...
              highlight_script_block(ani_p.script_index - 1);
              self.script_cur_index = ani_p.script_index - 1;
            }
          }
      ...
      */
      ...
      if (!REVERSE_ANIMATION) {
        ...
        for (i = 0; i < num_draw; i++) {
          self.start++;
          if (i != num_draw - 1) self.draw(false, true);
          else self.draw(false, false);
        } 
      } else {
  ...
  */
  ...
  /* draw on the canvas */
  draw(skip_animation = false, skip_draw = false) {
    let l_or_c, obj;
    let i;
    ...
    for (l_or_c of this.connection_map.values()) {
      for (i = 0; i < l_or_c.length; i++) l_or_c[i].draw(skip_animation, skip_draw);
    }
    for (obj of this.obj_map.values()) {
      obj.draw(skip_animation, skip_draw);
    }
    reset_canvas_offset();
  }
...
class quadraticCurve {
  ...
  set_animation_property(p) {
    ...
    if ("visible" in p) this.visible = p.visible;
    if ("fade_out" in p && p.fade_out == false) delete p.fade_out;
    if ("fade_in" in p && p.fade_in == false) delete p.fade_in;
    if ("text" in p) p.tmp_text = this.text;
    

    p.tmp_ctx_prop = this.ctx_prop;
    p.ctx_prop = update_dict(this.ctx_prop, extract_ctx_prop(p));
    p.tmp_alpha = this.alpha;
  }
  ...
  draw(skip_animation = false, skip_draw = false) {
      ...
      for (i = 0; i < this.animations.length; i++) {
        ...
        if (p.ani_time >= p.start && p.ani_time <= p.end) {
          ...
          if ( (p.ani_time == p.start && !REVERSE_ANIMATION) || (p.ani_time == p.end && REVERSE_ANIMATION) ) {
            if ("type" in p) this.animation_type = p.type;
            this.ctx_prop = p.ctx_prop;
            if (!REVERSE_ANIMATION && "text" in p) {
              this.text = p.text;
            }
          } 
          ...
          if ( (p.ani_time == p.end && !REVERSE_ANIMATION) || 
            ...
            if ("fade_out" in p) {
              if (!REVERSE_ANIMATION) this.alpha = 1 - (p.ani_time - p.start + 1) / p.time;
              else if (p.time != 1) this.alpha = 1 - (p.ani_time - p.start - 1) / p.time;
              if (p.tmp_alpha != 0) this.alpha *= p.tmp_alpha;
            }
            if ("fade_in" in p) {
              if (!REVERSE_ANIMATION) this.alpha = (p.ani_time - p.start + 1) / p.time;
              else if (p.time != 1) this.alpha = (p.ani_time - p.start - 1) / p.time;
              if(p.tmp_alpha != 0) this.alpha *= p.tmp_alpha;
            }
            if ("text" in p && REVERSE_ANIMATION) {
              this.text = p.tmp_text;
            }
            //console.log(p.time);
            // if ("text" in p) {
            //   if (REVERSE_ANIMATION) this.text = p.tmp_text;
            //   else this.text = p.text;
            // }
        ...
        if ( p.ani_time < p.start || p.ani_time > p.end || 
          ...
          if ("fade_in" in p && !REVERSE_ANIMATION) { // fade in effect, we keep the ctx prop.
            this.ctx_prop = deep_copy(p.ctx_prop);
          }  else this.ctx_prop = p.tmp_ctx_prop;
          animation_done++;
    ...
    if (animation_done != 0 && animation_done == this.animations.length) {
      p = this.animations[0];
      if ("fade_in" in p && !REVERSE_ANIMATION) {}
      else this.ctx_prop = p.tmp_ctx_prop;
      this.animations = [];
    }


    if (skip_draw || this.p1.is_euqal(this.p2) || this.alpha == 0) return;
    ...
    if (this.line_type == "direct" || this.line_type == "directed") {
      t = this.cut_directed_line();
      if (this.draw_arrow) this.draw_angle(t);
    } else if (this.draw_arrow) this.draw_angle();
    ...
    if ("tmp_p1" in this) {
      ctx.moveTo(this.tmp_p1.x,this.tmp_p1.y);
      this.cp = this.control_point(this.tmp_p1, this.tmp_p2, this.h_scale, this.w_scale);
      ctx.quadraticCurveTo(this.cp.x, this.cp.y, this.tmp_p2.x, this.tmp_p2.y);
    } else {
  ...
  draw_text() {
    ...
    if (this.text_direction == "left") {
      ...
      pv = perpendicular_vector(p, this.get_xy(this.text_t + 0.00001));
      if (this.p1.y < this.p2.y) dx = -tw + pv.x;
    } else if (this.text_direction == "top") {
      pv = {x: 0, y: 0};
      dx = -tw / 2;
    } else {
      //console.log("text_direction must be left or right or top");
    }
...
class Circle {
  constructor(x, y, r, 
    ...
    this.label_padding = PADDING;
    this.label_fillStyle = "black";
    this.label_offset_x = 0;
    this.visible = true;
    this.alpha = 1;
    this.move_type = "absolute";
  }
  ...
  move (dx, dy) {
    let i, tmp_propagation, tmp_stop_propagation;
    this.x += dx;
    this.y += dy;
    ...
    if (this.propagation && this.visited == false) {
      ...
      for (i = 0; i < this.to.length; i++) {
        if (this.to[i] != this) {
          ...
          tmp_stop_propagation = this.to[i].stop_propagation;
          this.to[i].propagation = true;
          this.to[i].stop_propagation = this.stop_propagation;
          if(!this.to[i].visited && !this.stop_propagation[this.to[i].ref]) this.to[i].move(dx,dy);
          this.to[i].propagation = tmp_propagation;
          this.to[i].stop_propagation = tmp_stop_propagation;
        }
      }
      ...
      for (i = 0; i < this.from.length; i++) {
        if (this.from[i] != this) {
          ...
          tmp_stop_propagation = this.from[i].stop_propagation;
          this.from[i].propagation = true;
          this.from[i].stop_propagation = this.stop_propagation;
          ...
          this.from[i].stop_propagation = tmp_stop_propagation;
        }
      }
  ...
  set_animation_property(p) {
    ...
    if ("walk" in p) {
      p.copy = p.walk;
      p.copy.copy_value = false;
    }
    
    if ("p" in p) {
      if ("type" in p && p.type == "relative") {
        p.dx = p.p.x / p.time;
        p.dy = p.p.y / p.time;
      } else {
      ...
      if (!("copy_value" in p.copy)) p.copy.copy_value = true;
      // if (!("ctx_prop" in p.copy)) p.copy.ctx_prop = {};
      p.copy.tmp_text = p.copy.circle.text;
    } else if ("swap" in p) {
    ...
    if ("fade_in" in p && p.fade_in == false) delete p.fade_in;
    if ("label" in p) {
      if (!("color" in p.label)) p.label.color = this.label_fillStyle;
      if (!("text" in p.label)) p.label.text = this.label;
      p.label.tmp_text = this.label;
      p.label.tmp_color = this.label_fillStyle;
    }
    // //console.log(p);
    ...
    p.ctx_prop = update_dict(this.ctx_prop, extract_ctx_prop(p));
    p.tmp_alpha = this.alpha;
    // if ("fade_in" in p) p.tmp_ctx_prop = deep_copy(p.ctx_prop);
    
  }

  swap_entries(t, circle, h_scale, w_scale) {
    this.copy_to_entry(t, circle, h_scale, w_scale, true);
    circle.copy_to_entry(t, this, -h_scale, w_scale, true);
  }

  copy_to_entry(t, to_circle, h_scale, w_scale, copy_value = true) {
    let p, p1, p2;
    let l, c;
    ...
    p2 = new Point(to_circle.x, to_circle.y);  
    
    l = new quadraticCurve(p1, p2, h_scale, w_scale, false, {lineWidth: 5, strokeStyle: "rgb(255,0,0,0.5)"} );
    p = l.get_xy(t);
    c = new Circle(p.x, p.y, this.r);
    c.ctx_prop = deep_copy(this.ctx_prop);
    c.alpha = 0.3;
    if (copy_value == true) c.text = this.text;
  ...
  draw(skip_animation = false, skip_draw = false) {
      ...
      for (i = 0; i < this.animations.length; i++) {
        ...
        if (p.ani_time >= p.start && p.ani_time <= p.end) {
          ...
          if ( (p.ani_time == p.start && !REVERSE_ANIMATION) || (p.ani_time == p.end && REVERSE_ANIMATION) ) {
            ...
            if ("label" in p) {
              if (!REVERSE_ANIMATION) {
                this.label = p.label.text;
                this.label_fillStyle = p.label.color;
              } else {
                this.label = p.label.tmp_text;
                this.label_fillStyle = p.label.tmp_color;
              }
            }
          ...
          if ( (p.ani_time == p.end && !REVERSE_ANIMATION) || 
            ...
            if ("p" in p) { 
              ...
              if (!REVERSE_ANIMATION || 
                  (REVERSE_ANIMATION && p.copy.copy_value == false) ) {
                this.copy_to_entry((p.ani_time - p.start + 1) / p.time, 
                                    p.copy.circle, p.copy.h_scale, p.copy.w_scale, p.copy.copy_value);
              } 
            } else if ("swap" in p) {
              this.swap_entries((p.ani_time - p.start + 1) / p.time, p.swap.circle, p.swap.h_scale, p.swap.w_scale);
            }
        ...
        if ( p.ani_time < p.start || p.ani_time > p.end || 
          ...
          if ("copy" in p && p.copy.copy_value == true) {
           
            if (!REVERSE_ANIMATION) p.copy.circle.text = this.text;
            else p.copy.circle.text = p.copy.tmp_text;
          } else if ("swap" in p) {
            tmp = this.text;
            this.text = p.swap.circle.text;
            p.swap.circle.text = tmp;
          } 
    ...
    if (skip_draw || this.alpha == 0) return;
    ctx.save();
    ctx.globalAlpha = this.alpha;
    load_ctx_prop(this.ctx_prop);

    ctx.beginPath();
    // ctx.ellipse(x, y, r, r / 1.4, 0, 0, 2 * Math.PI);
    ...
    if (this.label !== "") {
      if (this.label_position == "top") this.draw_text(x - r, y - r - this.label_padding, 2 * r, this.label, true);
      else this.draw_text(x - r, y + r + this.label_padding, 2 * r, this.label, true);
    }
    ctx.restore();
  }
  ...
  draw_text(x, y_middle, width, text, draw_label = false) {
    let tw;

    if (draw_label) {
      ctx.fillStyle = this.label_fillStyle;
      ctx.font = this.label_font;
      x += this.label_offset_x;
    } else {
      ctx.fillStyle = "black";
      ctx.font = FONT;
    }
...
class Rect {
  ...
  constructor(x, y, width, height, ref = "", text = [], label = "", label_position = "top", text_direction = 'v', ctx_prop = {}) {
    ...
    this.subrect_labels = [];
    this.subrect_label_position = "bottom";
    this.second_subrect_labels = [];
  ...
  copy_to_entry(t, index1, rect, index2, h_scale, w_scale) {
    ...
    if (this === rect && index1 == index2) {
      p2.x += 10;
      p2.y += 0;
      // h_scale = 30;
    }
    //console.log(this.text.length, block_size, this.width, p1, p2, this, rect);
    l = new quadraticCurve(p1, p2, h_scale, w_scale, false, {lineWidth: 5, strokeStyle: "rgb(255,0,0,0.5)"} );
    p = l.get_xy(t);
  ...
  swap_entries(t, rect = null, index1, index2, h_scale = 2, w_scale = 0.5) {

    if (rect != null) {
      this.copy_to_entry(t, index1, rect, index2, h_scale, w_scale);
      rect.copy_to_entry(t, index2, this, index1, -h_scale, w_scale);
      ...
      this.copy_to_entry(t, index1, this, index2, h_scale, w_scale);
      this.copy_to_entry(t, index2, this, index1, -h_scale, w_scale);
    }
    ...
    if (rect != null) prop2.fillStyle = rect.fillStyles[index2];
    else prop2.fillStyle = this.fillStyles[index2];
  ...
  set_animation_property(p) {
    ...
    if ("p" in p) {
      ...
      if (!("h_scale" in p.swap)) p.swap.h_scale = 2;
      if (!("w_scale" in p.swap)) p.swap.w_scale = 0.5;
      if (!("rect" in p.swap)) p.swap.rect = null;
    } else if ("copy" in p) {
      if (!("h_scale" in p.copy)) p.copy.h_scale = 2;
      if (!("w_scale" in p.copy)) p.copy.w_scale = 0.5;
      if (!("ctx_prop" in p.copy)) p.copy.ctx_prop = {};
    ...
    if ("text_fade_in" in p) {
      p.text_fade_in.tmp_text = this.text[p.text_fade_in.index];
      if (! ("text" in p.text_fade_in)) p.text_fade_in.text = this.text[p.text_fade_in.index];
      if ("fillStyle" in p.text_fade_in) {
        p.text_fade_in.tmp_fillStyle =  this.fillStyles[p.text_fade_in.index];
      }
      if (! ("color" in p.text_fade_in)) p.text_fade_in.color = "black";

    }
    if ("text_fade_out" in p) {
      p.text_fade_out.text = this.text[p.text_fade_out.index];

      if ("fillStyle" in p.text_fade_out) {
        p.text_fade_out.tmp_fillStyle =  this.fillStyles[p.text_fade_out.index];
      }
      if (! ("color" in p.text_fade_out)) p.text_fade_out.color = "black";
    }

    if ("width" in p) p.tmp_width = this.width;
    if ("height" in p) p.tmp_height = this.height;
    if ("subrect_labels" in p) p.tmp_subrect_labels = this.subrect_labels;
  ...
  draw (skip_animation = false, skip_draw = false) {
      ...
      for (i = 0; i < this.animations.length; i++) {
        ...
        if (p.ani_time >= p.start && p.ani_time <= p.end) {
          ...
          if ( (p.ani_time == p.start && !REVERSE_ANIMATION) || (p.ani_time == p.end && REVERSE_ANIMATION) ) {
            ...
            if (!REVERSE_ANIMATION && "width" in p) this.width = p.width;
            if (!REVERSE_ANIMATION && "height" in p) this.height = p.height;
            if (!REVERSE_ANIMATION && "subrect_labels" in p) this.subrect_labels = p.subrect_labels;
          ...
          if ( (p.ani_time == p.end && !REVERSE_ANIMATION) || 
            ...
            if ("text_fade_out" in p) {
              if (!REVERSE_ANIMATION) alpha = 1 - (p.ani_time - p.start + 1) / p.time;
              else if (p.time != 1) alpha = 1 - (p.ani_time - p.start - 1) / p.time;
              ...
              if ("fillStyle" in p.text_fade_out) {
                this.fillStyles[p.text_fade_out.index] = p.text_fade_out.fillStyle;
              }
            }

            if ("text_fade_in" in p) {
              if (!REVERSE_ANIMATION) alpha = (p.ani_time - p.start + 1) / p.time;
              else if (p.time != 1) alpha = (p.ani_time - p.start - 1) / p.time;
              ...
              if ("fillStyle" in p.text_fade_in) {
                this.fillStyles[p.text_fade_in.index] = p.text_fade_in.fillStyle;
              }
            }
            ...
            if ("p" in p) { 
              ...
              this.swap_entries( t / p.time, p.swap.rect,
        ...
        if ( p.ani_time < p.start || p.ani_time > p.end || 
          ...
          if ("fade_in" in p && !REVERSE_ANIMATION) { // fade in effect, we keep the ctx prop.
            this.ctx_prop = deep_copy(p.ctx_prop);
            this.fillStyles = deep_copy(p.fillStyle);
          } else {
            this.ctx_prop = p.tmp_ctx_prop;
            this.fillStyles = p.tmp_fillStyles;
          }
          ...
          if ("swap" in p) {
            if (p.swap.rect != null)  {
              ...
              this.text[p.swap.index1] = p.swap.rect.text[p.swap.index2];
              p.swap.rect.text[p.swap.index2] = tmp;
            } else {
              tmp = this.text[p.swap.index1];
          ...
          if ("text_fade_in" in p) {
            if (REVERSE_ANIMATION) {
              // if (p.time != 1) this.text[p.text_fade_in.index] = "";
              // else this.text[p.text_fade_in.index] = p.text_fade_in.tmp_text;
              this.text[p.text_fade_in.index] = p.text_fade_in.tmp_text;

              if (p.text_fade_in.index in this.text_color) delete this.text_color[p.text_fade_in.index];
              if ("tmp_fillStyle" in p.text_fade_in) this.fillStyles[p.text_fade_in.index] = p.text_fade_in.tmp_fillStyle;
            }
          ...
          if ("width" in p && REVERSE_ANIMATION) this.width = p.tmp_width;
          if ("height" in p && REVERSE_ANIMATION) this.height = p.tmp_height;
          if ("subrect_labels" in p && REVERSE_ANIMATION) this.subrect_labels = p.tmp_subrect_labels;
    ...
    // fill the subrect
    // for (i = this.fillStyles.length; i < num_fields; i++) this.fillStyles.push(this.fillStyles[this.fillStyles.length - 1]);
    num_fields = this.text.length;
    j = (num_fields < this.fillStyles.length) ? num_fields : this.fillStyles.length;
    for (i = 0; i < j; i++) this.fill_sub_rect(i);

    j = (num_fields < this.subrect_labels.length) ? num_fields : this.subrect_labels.length;
    
    for (i = 0; i < j ; i++) {
      if (this.text_direction == "h") {
        if (this.subrect_label_position == "top") {
          this.draw_text(this.x + this.width / num_fields * i, this.y - PADDING, this.width / num_fields, this.subrect_labels[i], {fillStyle:"red", alpha: this.alpha}, true);
        } else {
          this.draw_text(this.x + this.width / num_fields * i, this.y + this.height + PADDING, this.width / num_fields, this.subrect_labels[i], {fillStyle:"red", alpha: this.alpha}, true);
        }
      }
    }

    j = (num_fields < this.second_subrect_labels.length) ? num_fields : this.second_subrect_labels.length;
    for (i = 0; i < j ; i++) {
      if (this.text_direction == "h") {
        // we use the opposite of postion for second_subrect_labels.
        if (this.subrect_label_position == "top") {
          this.draw_text(this.x + this.width / num_fields * i, this.y + this.height + PADDING, this.width / num_fields, this.second_subrect_labels[i], {fillStyle:"red", alpha: this.alpha}, true);
        } else {
          this.draw_text(this.x + this.width / num_fields * i, this.y - PADDING, this.width / num_fields, this.second_subrect_labels[i], {fillStyle:"red", alpha: this.alpha}, true);
        }
      }
    }
    ...
    for (i = 0; i < this.text.length; i++) {
      ...
      let draw_text_prop = {fillStyle: "black", alpha: this.alpha};
    ...
    if (this.label !== "") {
      if (this.label_position == "top") {
        this.draw_text(this.x, this.y - PADDING, this.width, this.label, {fillStyle:"black", alpha: this.alpha});
      } else if (this.label_position == "bottom"){
        this.draw_text(this.x, this.y + this.height + PADDING, this.width, this.label, {fillStyle:"black", alpha: this.alpha});
      } else if (this.label_position == "left") {
        tmp = ctx.measureText(this.label).width + 2 * PADDING;
        this.draw_text(this.x - tmp, this.y + this.height / 2, tmp, this.label, {fillStyle:"black", alpha: this.alpha});
  ...
  draw_text(x, y_middle, width, text, ctx_prop = {fillStyle: "black", alpha: 1}, draw_subrect_label = false) {
    let tw;
    ...
    ctx.textBaseline = "middle";
    if (draw_subrect_label) ctx.font = "11px Arial";
    else ctx.font = this.font;

    tw = ctx.measureText(text).width;

    if (this.text_align == "middle") {
      x = x + (width - tw) / 2;
    } else if (this.text_align == "left") {
      x = x + PADDING;
    } else if (this.text_align == "right") {
      x = x + width - tw - PADDING;
    }
...
class Text {
  constructor(text, x, y, width, font = FONT, ctx_prop = {}) {
    ...
    this.ctx_prop = update_dict(DEFAULT_TEXT_CTX, ctx_prop);
    this.text_align = "middle";
  }
  ...
  set_animation_property(p) {
    ...
    if ("text" in p) {
      p.tmp_text = this.text;
      //console.log(p);
    }
    if ("visible" in p) this.visible = p.visible;
  }
  draw(skip_animation = false, skip_draw = false) {
    let animation_done, i, p;
    let tw;
    ...
    if (skip_draw || this.alpha == 0 || !this.visible || this.text == "") return;
    
    ctx.save();
    ctx.globalAlpha = this.alpha;
    ctx.fillStyle = this.ctx_prop.fillStyle;
    ctx.textBaseline = "middle";
    ctx.font = this.font;
...
class Graph {
  constructor(ani, graph_type = "undirect", non_removable_nodes = []) {
    ...
    this.offset_x = MAIN_G_SPEC.offset_x;
    this.offset_y = MAIN_G_SPEC.offset_y;
   
  }


  save_original_graph() {
    let save_g = new Graph(new Animation(), this.graph_type);
    let n, e, n2, new_e;
    let id1, id2;
    for (let key in this.node_map) {
      n = this.node_map[key];
      n2 = save_g.get_node(n.id, n.ani_circle.x, n.ani_circle.y);
      n2.ani_circle.ctx_prop = deep_copy(n.ani_circle.ctx_prop);
    }

    for (let key in this.edge_map) {
      e = this.edge_map[key];
      n = save_g.get_node(e.n1.id);
      n2 = save_g.get_node(e.n2.id);
      new_e = save_g.get_edge(n, n2, e.weight);
      new_e.ani_line.ctx_prop = deep_copy(e.ani_line.ctx_prop);
      new_e.ani_line.text_direction = e.ani_line.text_direction;
      new_e.ani_line.text_t = e.ani_line.text_t;
    }

    this.original_graph = save_g;
  }
  ...
  deep_copy(ani = null) {
    ...
    g.non_removable_nodes = this.non_removable_nodes;
    g.original_graph = this.original_graph;
  ...
  get_node_position() {
    ...
    if (this.layout == "grid") {
      ...
      while (1) {
        ...
        col = i % this.num_cols;
        y = row * this.gap_y + this.offset_y;
        x = col * this.gap_x + this.offset_x;
        ok = true;
        for (let key in this.node_map) {
  ...
  remove_edge(n1, n2) {
    ...
    if ((this.graph_type == "undirect" || this.graph_type == "undirected") && id2 in this.edge_map) {
      e = this.edge_map[id2];
  ...
  get_nodes(ids) {
    ...
    for (let i = 0; i < ids.length; i++) {
      nodes.push(this.get_node(ids[i]));
    }
    return nodes;
  }
  get_node (id, x = null, y = null, name = null, force = false) {
    ...
    base_id = id;
    if (!force && id in this.node_map) {
      if (x != null && y != null) {
        n = this.node_map[id];
        n.ani_circle.move(x - n.ani_circle.x, y - n.ani_circle.y);
      }
      return this.node_map[id];
    }
    if (force) {
      i = 0;
      while (id in this.node_map) {
        id = base_id + "_DUPLICATE_" + i;
        i++;
      }
    }
  ...
  get_edge_by_name(id1, id2, weight = 0, force = true) {
    ...
    if (this.is_node(id1)) n1 = this.get_node(id1);
    if (this.is_node(id2)) n2 = this.get_node(id2);
    // //console.log(this.is_edge(n1,n2), n1.id, n2.id);
    if (n1 == null || n2 == null) {
      return null;
    } else return this.get_edge(n1, n2, weight, force);
  }

  get_edge_if_exists(n1, n2, weight = 0, force = true, line = null) {
    if (this.is_edge(n1, n2)) return this.get_edge(n1, n2, weight, force, line);
    else return null;
  }


  get_edge(n1, n2, weight = 0, force = true, line = null) {
    let id = n1.id + "->" + n2.id;
    let id2 = n2.id + "->" + n1.id;
    let e, l;
    let l_or_c;
    ...
    if (line != null) l = line;
    else if (l_or_c.length != 0) l = l_or_c[0];
    else {
  ...
  DFS_cycle_detection(n) {

    n.visited = 1;
    n.stack = 1;
    for (let i = 0; i < n.adj.length; i++) {
      let n2 = n.adj[i].n2;
      if(n2.visited == 0 && this.DFS_cycle_detection(n2)) return true;
      else if (n2.stack == 1) return true;
    }
    n.stack = 0;
    return false;
  }
  ...
  cycle_detection() {
    let node_map = this.node_map;
    for (let key in node_map) {
      node_map[key].visited = 0;
      node_map[key].stack = 0;
    }
    for (let key in node_map) {
      let n = node_map[key];
      if (n.visited != 1) {
        if (this.DFS_cycle_detection(n)) return true;
      }
    }
