...
class stackAnimation {
  ...
  push(v) {
    ...
    if (stack.size >= this.max_size) {
      $("#elaboration_text").text("Cound't push. We limit the max size of stack to {}".format(this.max_size));
      return;
    } else if (input_empty_check(v, false)) {
    ...
    y = R_OFFSET_Y;
    rect = new Rect(x, y, R_WIDTH, R_HEIGHT, ref, ["val = " + add_quotes(v), "next = NULL"], ref, "bottom");
    ani.add_object(rect);
