  ...
  insertion_sort(v, rect, x, y, t_rect, size) {
    ...
    for(i = 1; i < v_size; i++){
      ...
      if(j !== i){
        ...
        ani.add_sequence_ani({ 
          target: rect,
          text: "Element {}{} array[{}]</span> is greater than {}{} array[{}]</span>.".format(RED_SPAN, tmp, j, BLUE_SPAN, v_c[j-1], (j - 1)),
          prop: {
            fade_in: true,
            fillStyle: deep_copy(fill_styles),
            step: true,
        ...
        ani.add_sequence_ani({
          target: t_rect,
          text: "Therefore restore {}tmp {}</span> into {}(array[{}])</span>.".format(BLUE_SPAN, tmp, BLUE_SPAN, j),
          prop: {
            copy: {index1: 0, rect: rect, index2: j, h_scale: 0, w_scale: 0},
            fillStyle: "lightblue",
            time: 1.5 * ANIMATION_TIME, 
